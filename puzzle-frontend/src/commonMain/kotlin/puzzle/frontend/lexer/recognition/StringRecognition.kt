...
import puzzle.token.kinds.StringKind.Template.Part

internal object StringRecognition : TokenRecognition {
	...
	private fun parseText(input: CharArray, start: Int, isMultiLine: Boolean): PzlToken {
		...
		while (position < input.size) {
			position += when (parseCharType(input, position, isMultiLine)) {
				...
				ESCAPE -> 2
				UNICODE -> 6
				NEWLINE -> if (isMultiLine) 1 else lexicalError("请使用多行字符串", position)
				else -> break
			}
		}
	...
	private fun parseTemplate(input: CharArray, start: Int, isMultiLine: Boolean, dollarCount: Int): PzlToken {
		...
		while (position < input.size) {
			...
			position += when (type) {
				...
				NEWLINE -> if (isMultiLine) 1 else lexicalError("请使用多行字符串模版", position)
					...
					val tokens = when (type) {
						SIMPLE_INTERPOLATION -> {
							...
							listOf(token, PzlToken(EOF, token.location.end span token.location.end + 1))
						}
						...
						EXPRESSION_INTERPOLATION -> {
							...
							if (tokens.isEmpty()) {
								lexicalError("字符串插值缺少表达式", position)
							}
				...
				when (val escape = input[position + 1]) {
					...
					UNICODE_CODE -> {
						val unicode = input.safeString(position + 2, 4)
							?: lexicalError("语法错误", position)
						if (!unicode.isHex()) {
							lexicalError("非法转译字符: '${input.safeString(position, 6)}'", position)
						}
						UNICODE
					}
					
					else -> lexicalError("非法转译字符: '\\$escape'", position)
				}
			}

...
import puzzle.ast.declaration.MultiReturnSpec
import puzzle.ast.declaration.SingleReturnSpec
...
import puzzle.frontend.parser.parser.parameter.context.parseLambdaContextSpec
import puzzle.frontend.parser.parser.parameter.parameter.parseLambdaParameters
		...
		val types = buildList {
			while (!cursor.match(RBRACKET)) {
				...
				if (!cursor.check(RBRACKET)) {
					cursor.expect(COMMA, "lambda 表达式多返回值列表缺少 ','")
				}
			}
		}
		if (types.isEmpty()) {

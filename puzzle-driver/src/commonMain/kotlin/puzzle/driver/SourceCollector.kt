...
object SourceCollector {
    ...
    fun collect(projectPath: FilePath) {
        ...
        RootContext.projects.checkModuleGroupRepeatability()
    ...
    private fun List<ProjectContext>.checkModuleGroupRepeatability() {
        val groups = mutableSetOf<List<String>>()
        this.forEach { project ->
            project.modules.forEach { module ->
                configCheck(module.group !in groups) {
                    configError("'${module.group.joinToString(".")}' 组重复了")
                }
                groups += module.group
            }
        }
    ...
    private fun getProjectConfig(projectPath: FilePath, isRootProject: Boolean): ProjectConfig {
        ...
        val name = config.name ?: configError("项目名未配置", "name", path = configPath)
        configCheck(name.isNotBlank()) {
            configError("项目名不能为空", "name", path = configPath)
        }
        configCheck(name.isValidName()) {
            configError("项目名格式错误, 只允许包含 'a'-'z' 'A'-'Z' '-'", "name", path = configPath)
        }

        val version = config.version ?: configError("版本号未配置", "version", path = configPath)
        configCheck(version.isNotBlank()) {
            configError("版本号不能为空", "version", path = configPath)
        }
        configCheck(version.isValidVersion()) {
            configError("版本号格式错误, 正确格式: 大版本.小版本.补丁号", "version", version, configPath)
        }

        val modules = config.modules ?: configError("模块列表未配置", "modules", path = configPath)
        configCheck(modules.isNotEmpty()) {
            configError("模块列表不能为空", "modules", path = configPath)
        }

        if (!isRootProject) return config

        val entry = config.entry ?: configError("入口模块未配置", "entry", path = configPath)
        configCheck(entry.isNotBlank()) {
            configError("入口模块不能为空", "entry", path = configPath)
        }
        configCheck(entry in modules) {
            configError("入口模块未在模块列表中找到", "entry", entry, configPath)
        }
        return config
    }
    ...
    private fun getModuleConfig(modulePath: FilePath, projectConfig: ProjectConfig): ModuleConfig {
        ...
        val name = config.name ?: configError("模块名未配置", "name", path = configPath)
        configCheck(name.isNotBlank()) {
            configError("模块名不能为空", "name", path = configPath)
        }
        configCheck(name.isValidName()) {
            configError("模块名格式错误, 只允许包含 'a'-'z' 'A'-'Z' '-'", "name", name, configPath)
        }
        ...
        val group = config.group ?: configError("组未配置", "group", path = configPath)
        configCheck(group.isValidGroup()) {
            configError("模块组格式错误", "group", group, configPath)
        }
        ...
        return ModuleContext().apply {
            ...
            val deps = config.deps?.mapIndexed { index, dep ->
                ...
                val projectConfig = projectConfigs.find { it.name == projectName }
                    ?: configError("依赖不存在", "deps[$index]", dep, path)
    ...
    private fun getAllSourcePaths(
        sourcePath: FilePath,
        fileRules: List<IgnoreRule>,
        dirRules: List<IgnoreRule>
    ): List<FilePath> {
        if (!sourcePath.exists) return emptyList()
        return when {
            sourcePath.isFile && sourcePath.name.endsWith(".pzl") -> if (sourcePath.isIgnoreFile(fileRules)) emptyList() else listOf(
                sourcePath
            )
    ...
    private fun List<String>.toIgnoreRules(modulePath: FilePath): Set<IgnoreRule> {
        ...
        return this.mapIndexed { index, ignore ->
            when {
                ...
                ignore.endsWith("/**") -> IgnoreRule(
                    path = "$path/${ignore.removeSuffix("/**")}",
                ...
                ignore.isBlank() -> configError(
                    message = "规则不能为空",
                    name = "ignore[$index]",
                    path = path(modulePath, "puzzle.json")
                )

                else -> configError(
                    message = "忽略规则错误, 规则示例: '**', '*', 'src/main/puzzle/*', 'src/main/puzzle/**', 'src/main/puzzle/String.pzl'",
                    name = "ignore[$index]",
                    value = ignore,
                    path = path(modulePath, "puzzle.json")
                )
        ...
        val message = buildString {
            ...
            projects.forEachIndexed { projectIndex, project ->
                ...
                modules.forEachIndexed { moduleIndex, module ->
                    ...
                    val ignoreRules = module[IgnoreRuleAttachment::class].values
